...
if ( ! function_exists( 'ere_register_property_post_type' ) ) {
	...
	function ere_register_property_post_type() {
		...
		$property_post_type_args = array(
			...
			'show_in_rest'       => true,
			'rest_base'          => apply_filters( 'inspiry_property_rest_base', __( 'properties', 'easy-real-estate' ) )
		);
...
if ( ! function_exists( 'ere_register_property_city_taxonomy' ) ) :
	function ere_register_property_city_taxonomy() {
		...
		$city_labels = array(
			'name'                       => esc_html__( 'Property Cities', 'easy-real-estate' ),
			'singular_name'              => esc_html__( 'Property City', 'easy-real-estate' ),
			'search_items'               => esc_html__( 'Search Property Cities', 'easy-real-estate' ),
			'popular_items'              => esc_html__( 'Popular Property Cities', 'easy-real-estate' ),
			'all_items'                  => esc_html__( 'All Property Cities', 'easy-real-estate' ),
			'parent_item'                => esc_html__( 'Parent Property City', 'easy-real-estate' ),
			'parent_item_colon'          => esc_html__( 'Parent Property City:', 'easy-real-estate' ),
			'edit_item'                  => esc_html__( 'Edit Property City', 'easy-real-estate' ),
			'update_item'                => esc_html__( 'Update Property City', 'easy-real-estate' ),
			'add_new_item'               => esc_html__( 'Add New Property City', 'easy-real-estate' ),
			'new_item_name'              => esc_html__( 'New Property City Name', 'easy-real-estate' ),
			'separate_items_with_commas' => esc_html__( 'Separate Property Cities with commas', 'easy-real-estate' ),
			'add_or_remove_items'        => esc_html__( 'Add or remove Property Cities', 'easy-real-estate' ),
			'choose_from_most_used'      => esc_html__( 'Choose from the most used Property Cities', 'easy-real-estate' ),
			'menu_name'                  => esc_html__( 'Property Cities', 'easy-real-estate' ),
		);
...
if ( ! function_exists( 'ere_set_property_city_slug' ) ) :
	 ...
	 *
	 * @param string $existing_slug - Existing property city slug.
	 *
	 * @return string
	 */
...
if ( ! function_exists( 'ere_property_edit_columns' ) ) {
	...
	function ere_property_edit_columns( $columns ) {
		...
		$columns = array(
			...
			'city'               => esc_html__( 'City', 'easy-real-estate' ),
		...
		switch ( $column ) {
			case 'property-thumbnail':
				if ( has_post_thumbnail( $post->ID ) ) {
					...
					_e( 'No Thumbnail', 'easy-real-estate' );
				}
				break;
			case 'id':
...
if ( ! function_exists( 'ere_property_rest_api_meta_field' ) ) {

	add_action( 'rest_api_init', 'ere_property_rest_api_meta_field' );

	function ere_property_rest_api_meta_field() {
		register_rest_field( 'property', 'property_meta', array(
			'get_callback' => 'ere_get_property_meta_for_rest_api'
		) );
	}

	function ere_get_property_meta_for_rest_api( $object ) {
		// get the id of the post object array
		$post_id = $object['id'];

		// get post meta
		$property_post_meta = get_post_meta( $post_id );

		// filter the post meta
		$property_post_meta = apply_filters( 'ere_property_rest_api_meta', $property_post_meta );
...
if ( ! function_exists( 'ere_properties_filter_fields_admin' ) ) {
	...
	function ere_properties_filter_fields_admin() {
		...
		if ( $post_type == 'property' ) {
			...
			// Property City Dropdown Option
			$prop_city_args = array(
				'show_option_all' => esc_html__( 'All Property Cities', 'easy-real-estate' ),
				'orderby'         => 'NAME',
				'order'           => 'ASC',
	...
	function ere_properties_filter_admin( $query ) {
		...
		if ( $pagenow == 'edit.php' && $post_type == 'property' ) {
			...
			if ( isset( $_GET['property_city_admin_filter'] ) && ! empty( $_GET['property_city_admin_filter'] ) ) {
				...
				//get the desired property city
				$property_city = sanitize_text_field( $_GET['property_city_admin_filter'] );

				//if the property city is not 0 (which means all)
				if ( $property_city != 0 ) {
					$tax_query[] = array(
						'taxonomy' => 'property-city',
						'field'    => 'ID',
