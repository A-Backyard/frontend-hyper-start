		...
		public function settings() {
			...
			if(isset($_POST['wp_cdnjs_uninstall'])) {
				...
				<div class="updated">
					<p><?php _e('All options have been removed from the database.', 'wp-cdnjs'); ?>
						...
						<?php 
            // printf(__('To complete the uninstall <a href="%1$s">deactivate %2$s.</a>', 'wp-cdnjs'), esc_url($deactivate_url), WP_CDNJS_PLUGIN_NAME); 
            printf('%1$s <a href="%1$s"">%3$s %4$s.</a>', __('To complete the uninstall', 'wp-cdnjs'), esc_url($deactivate_url), __('deactivate', 'wp-cdnjs'), WP_CDNJS_PLUGIN_NAME); 
            ?>
					</p>
				</div>
				<?php
			...
			<div id="wp-cdnjs-reset" style="display:none; clear: both;">
				<form method="post" action="">
					...
					<label style="font-weight:normal;">
						<?php printf(__('Do you wish to completely reset the default options for', 'wp-cdnjs')); ?> <?php echo WP_CDNJS_PLUGIN_NAME ?>? </label>
			...
			<div id="wp-cdnjs-uninst" style="display:none; clear: both;">
				<form method="post" action="">
					...
					<label style="font-weight:normal;">
						<?php echo sprintf(__('Do you wish to completely uninstall', 'wp-cdnjs')); ?> <?php echo WP_CDNJS_PLUGIN_NAME ?>?</label>
